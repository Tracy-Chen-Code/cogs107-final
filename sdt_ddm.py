...
# debug with the help of ChatGPT
...
# ---------- Optional README print ----------
readme = ROOT / "README.md"
if readme.exists():
    with open(readme, "r", encoding="utf-8") as f:
        print(f.read())
else:
    print(f"[INFO] {readme} not found; skipping README print.")
...
def apply_hierarchical_sdt_model(data, draws=2000, tune=1000, target_accept=0.9, seed=42):
    ...
    P = data["pnum"].nunique()
    conds = np.sort(data["condition"].unique())
    C = len(conds)

    diff_code  = np.array([c // 2 for c in conds])   # 0/1
    stim_code  = np.array([c %  2 for c in conds])   # 0/1
    inter_code = diff_code * stim_code

    with pm.Model() as model:
        # group-level effects on d′
        alpha_d  = pm.Normal("alpha_d", 0, 1)
        beta_s   = pm.Normal("beta_s",  0, 1)
        beta_d   = pm.Normal("beta_d",  0, 1)
        beta_int = pm.Normal("beta_int",0, 1)

        # group-level effects on criterion
        alpha_c  = pm.Normal("alpha_c", 0, 1)
        gamma_s  = pm.Normal("gamma_s", 0, 1)
        gamma_d  = pm.Normal("gamma_d", 0, 1)
        gamma_int= pm.Normal("gamma_int",0, 1)

        mu_d  = alpha_d + beta_s*stim_code + beta_d*diff_code + beta_int*inter_code
        mu_c  = alpha_c + gamma_s*stim_code + gamma_d*diff_code + gamma_int*inter_code
        ...
        hit_p = pm.math.invlogit(d_prime - criterion)
        fa_p  = pm.math.invlogit(-criterion)

        pm.Binomial("hits_obs",
                    n=data["nSignal"],
                    p=hit_p[data["pnum"]-1, data["condition"]],
                    observed=data["hits"])
        pm.Binomial("fa_obs",
                    n=data["nNoise"],
                    p=fa_p[data["pnum"]-1, data["condition"]],
    ...
    summ = az.summary(trace,
                      var_names=["alpha_d","beta_s","beta_d","beta_int",
...
if __name__ == "__main__":
    # ---------- Step 1: read & show SDT table ----------
    sdt_df = read_data(DATA_PATH, prepare_for="sdt", display=True)

    # ---------- Step 2: fit hierarchical SDT ----------
    model, trace, summ = apply_hierarchical_sdt_model(sdt_df)
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR/"forest.png", dpi=300)
    plt.close()

    # Pair plot (d′ params)
    az.plot_pair(trace,
                 var_names=["alpha_d","beta_s","beta_d","beta_int"],
                 kind="scatter", divergences=False)
    plt.suptitle("Pair plot – d′ parameters", y=1.02)
    plt.savefig(OUT_DIR/"pairplot_dprime.png", dpi=300)
    plt.close()

    # Trace plot (all group-level params)
    az.plot_trace(trace,
                  var_names=["alpha_d","beta_s","beta_d","beta_int",
                             "alpha_c","gamma_s","gamma_d","gamma_int"],
                  figsize=(12, 20), compact=True)
    plt.suptitle("Trace plots – group-level parameters", y=0.995)
    plt.tight_layout()
    ...
    print(f"[INFO] Added figure links to {readme_path}")
